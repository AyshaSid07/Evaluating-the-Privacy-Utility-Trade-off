...
def train_model(X, Y, model, verbose=True):
    ...
    if use_weights:
        ...
        print('  *', cls_type, 'Precision:', 
              sklearn.metrics.precision_score(labels_test, pred_test, sample_weight=weights_test))
        print('  *', cls_type, 'Recall:', 
              sklearn.metrics.recall_score(labels_test, pred_test, sample_weight=weights_test))
        print('  *', cls_type, 'F1 score:', 
              sklearn.metrics.f1_score(labels_test, pred_test, sample_weight=weights_test))
    else:
        print('  *', cls_type, 'accuracy:', 
              sklearn.metrics.accuracy_score(labels_test, pred_test))
        print('  *', cls_type, 'Precision:', 
              sklearn.metrics.precision_score(labels_test, pred_test))
        print('  *', cls_type, 'Recall:', 
              sklearn.metrics.recall_score(labels_test, pred_test))
        print('  *', cls_type, 'F1 score:', 
              sklearn.metrics.f1_score(labels_test, pred_test))
