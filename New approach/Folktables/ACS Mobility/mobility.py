...
al_features, al_labels, _ = ACSMobility.df_to_pandas(acs_data)
combined_df = pd.concat([al_features, al_labels], axis=1)
combined_df.to_csv('folktables_mobility_RAW.csv', index=False)

# Split the data into training and testing sets and use a logistic regression model to predict mobility status
