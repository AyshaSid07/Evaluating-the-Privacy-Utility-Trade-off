...
data = pd.read_csv("../folktables_mobility_RAW.csv")
data.columns = data.columns.str.strip()

# 2. ACSMobility Configuration

cols = ["AGEP", "SCHL", "MAR", "SEX", "ESP", "CIT", "MIL", "ANC", "NATIVITY", "RELP", "GCL", "COW", "ESR", "WKHP", "JWMNP", "DIS", "DEAR", "DEYE", "DREM"]

# Fix the floats ("30.0" -> "30") in the dataset
for col in cols:
	data[col] = data[col].astype(float).astype(int).astype(str).str.strip()

quasi_ident = ["AGEP", "SCHL", "MAR", "SEX", "ESP", "CIT", "MIL", "ANC", "NATIVITY", "RELP", "GCL", "COW", "ESR", "WKHP", "JWMNP", "DIS", "DEAR", "DEYE", "DREM"]
...
sensitive = ["PINCP", "RAC1P"]

# also drop sensitive for now
...
data_anon.to_csv("mobility_k=10.csv", index=False)
