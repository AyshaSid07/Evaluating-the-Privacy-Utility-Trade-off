...
def get_utility_target_model(df_train_real_raw, df_test_real_raw, df_protected_original, seed, drop_sens=False):
    ...
    for c in ["index", "Linkage_Index"]:
        if c in df_train_real.columns: df_train_real = df_train_real.drop(columns=[c])
        if c in df_test_real.columns: df_test_real = df_test_real.drop(columns=[c])
...
def  encode_adversary_features(df, preprocessor=None, fit=False, filter_sensitive=True, df_train_real_raw=None):
    ...
    if filter_sensitive:
        ...
        sens = pd.to_numeric(sens_raw, errors='coerce').fillna(SUPPRESSED_CATEGORY_CODE).astype(int).values
...
df_train_real.columns = df_train_real.columns.str.strip()
df_test_real.columns = df_test_real.columns.str.strip()
majority_class_rate = df_test_real[df_test_real[SENSITIVE_ATTR].astype(str) != '*'][SENSITIVE_ATTR].astype(float).astype(int).value_counts(normalize=True).max()
...
for seed in seeds:
    ...
    num_cols_count = len([c for c in NUMERIC_COLS if c in df_train_real.columns])
    df_universal = pd.concat([df_train_real, df_test_real] + [d for d in datasets_to_test.values() if d is not None], ignore_index=True)
    ...
    X_test_encoded, y_test, sens_test, _ = encode_adversary_features(df_test_real, preprocessor=universal_prep, filter_sensitive=True)
    actual_flat = sens_test.flatten()

    model_b_report, X_train_b_report, X_test_b_report, y_test_b_report = get_utility_target_model(df_train_real, df_test_real, None, seed, drop_sens=False)
    ...
    model_ablation, _, X_test_abl, y_test_abl = get_utility_target_model(df_train_real, df_test_real, None, seed, drop_sens=True)
    ...
    for name, df_adv_original in datasets_to_test.items():
        ...
        result = get_utility_target_model(df_train_real, df_test_real, df_pass, seed, drop_sens=False)
        if result[0] is None:
            print(f"Skipping {name}: no valid training records after filtering.", flush=True)
            continue
        model_a_report, X_train_a_report, X_test_a_report, y_test_a_report = result
        ...
        df_to_prep = df_train_real if df_pass is None else df_pass
        X_adv_encoded, y_adv, sens_adv, _ = encode_adversary_features(
            df_to_prep, preprocessor=universal_prep, filter_sensitive=True,
            df_train_real_raw=(df_train_real if df_pass is not None else None)
        )
        ...
        if len(X_train_a_report) != X_adv_with_sens.shape[0]:
            print(f"  Row count mismatch between the adversary model ({X_adv_with_sens.shape[0]}) "
        ...
        FEATURE_COLS_B = [c for c in df_train_real.columns if c not in ["index", "Linkage_Index", PREDICTION_TARGET]]
        X_adv_query_b = build_scenario_b_query_features(df_train_real, df_pass, FEATURE_COLS_B, train_medians_b, drop_sens=False)
        ...
        pd.DataFrame(all_results).to_csv("../results/aia/credit_card_aia_results_all_runs_partial.csv", index=False)

pd.DataFrame(ablation_results).to_csv("../results/aia/credit_card_aia_limitcategory_ablation.csv", index=False)
results_df = pd.DataFrame(all_results)
results_df.to_csv("../results/aia/credit_card_aia_results_all_runs.csv", index=False)
summary_df = (
    results_df
...
summary_df.to_csv("../results/aia/credit_card_aia_results_summary.csv", index=False)
