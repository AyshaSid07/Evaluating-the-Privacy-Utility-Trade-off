...
def preprocess(df, preprocessor=None, fit=False, qi_only=False):
    ...
    df_clean = df_clean[df_clean[SENSITIVE_ATTR].astype(str) != '*']
    df_clean = df_clean[df_clean[PREDICTION_TARGET].astype(str) != '*']
...
df_train_real_raw = pd.read_csv('../datasets/MEPS_train.csv')
df_test_real_raw = pd.read_csv('../datasets/MEPS_test.csv')
