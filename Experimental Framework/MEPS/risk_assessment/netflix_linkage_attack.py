...
if __name__ == "__main__":
    df_original = pd.read_csv('../datasets/MEPS_train.csv')
    ...
    for run_id, seed in enumerate(seeds, start=1):
        ...
        # Sample targets from original training data
        ...
        pd.DataFrame(all_summaries).to_csv(
            "../results/linkage/linkage_attack_results_meps_all_runs_partial.csv",
            index=False
        )

    results_df = pd.DataFrame(all_summaries)
    results_df.to_csv("../results/linkage/linkage_attack_results_meps_all_runs.csv", index=False)
    ...
    summary_df.to_csv("../results/linkage/linkage_attack_results_meps_summary.csv", index=False)
