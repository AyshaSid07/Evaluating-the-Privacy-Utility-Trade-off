...
n_attacker_records = 3800 # 1% of the dataset size
...
if __name__ == "__main__":
    df_original = pd.read_csv('../datasets/folktables_employment_train.csv')
    ...
    for run_id, seed in enumerate(seeds, start=1):
        ...
        pd.DataFrame(all_summaries).to_csv(
            "../results/linkage/linkage_attack_results_employment_all_runs_partial1.csv",
            index=False
        )

    results_df = pd.DataFrame(all_summaries)
    results_df.to_csv("../results/linkage/linkage_attack_results_employment_all_runs1.csv", index=False)
    ...
    summary_df.to_csv("../results/linkage/linkage_attack_results_employment_summary1.csv", index=False)
