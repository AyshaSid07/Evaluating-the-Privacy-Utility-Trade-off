...
df_train_real = pd.read_csv('../datasets/folktables_employment_train.csv')
df_test_real = pd.read_csv('../datasets/folktables_employment_test.csv')
...
for run_id, seed in enumerate(seeds, start=1):
    ...
    for config_name, df_protected in selected_configs.items():
        ...
        output_path = f"../results/dcr/record_level/employment_{safe_name}_run_{run_id}.csv"
...
summary_df.to_csv("../results/dcr/dcr_results_employment_all_runs.csv", index=False)

agg_summary = summary_df.groupby("Configuration").agg(["mean", "std"], numeric_only=True)
agg_summary.to_csv("../results/dcr/dcr_results_employment_summary.csv")
