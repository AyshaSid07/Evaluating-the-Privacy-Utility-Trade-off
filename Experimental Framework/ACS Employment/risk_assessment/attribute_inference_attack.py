...
QI_COLS = ["AGEP", "SCHL", "SEX", "MAR", "RELP", "ESP", "CIT", "MIG", "MIL", "ANC", "NATIVITY"]
NUMERIC_COLS = ['AGEP']
DATASET_NAME = "ACS Employment"
...
df_train_real_raw = pd.read_csv('../datasets/folktables_employment_train.csv', low_memory=False)
df_test_real_raw = pd.read_csv('../datasets/folktables_employment_test.csv', low_memory=False)
...
for seed in seeds:
    ...
    for name, df_adv_original in datasets_to_test.items():
        ...
        if len(X_adv_query_b) != X_adv_with_sens.shape[0]:
            print(f"  WARNING: row count mismatch for scenario B on {name}; skipping.", flush=True)
            continue
        ...
        pd.DataFrame(all_results).to_csv("../results/aia/employment_aia_results_all_runs_partial.csv", index=False)
pd.DataFrame(ablation_results).to_csv("../results/aia/employment_aia_rac1p_ablation.csv", index=False)
results_df = pd.DataFrame(all_results)
results_df.to_csv("../results/aia/employment_aia_results_all_runs.csv", index=False)
summary_df = (
    results_df
...
summary_df.to_csv("../results/aia/employment_aia_results_summary.csv", index=False)
