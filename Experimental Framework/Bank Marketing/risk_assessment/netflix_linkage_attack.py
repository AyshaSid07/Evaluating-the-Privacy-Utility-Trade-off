...
n_attacker_records = 400 # 1% of the dataset size
...
if __name__ == "__main__":
    df_original = pd.read_csv('../datasets/bank_marketing_train.csv')
    ...
    for run_id, seed in enumerate(seeds, start=1):
        ...
        # K3 Fix: Sample targets from original training data and do not filter out suppressed ones.
        ...
        pd.DataFrame(all_summaries).to_csv(
            "../results/linkage/linkage_attack_results_bank_marketing_all_runs_partial.csv",
            index=False
        )

    results_df = pd.DataFrame(all_summaries)
    results_df.to_csv("../results/linkage/linkage_attack_results_bank_marketing_all_runs.csv", index=False)
    ...
    summary_df.to_csv("../results/linkage/linkage_attack_results_bank_marketing_summary.csv", index=False)
