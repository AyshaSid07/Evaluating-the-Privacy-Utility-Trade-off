...
SUPPRESSED_CATEGORY_CODE = 3  # Distinct code for suppressed ('*') sensitive-attribute values, used only when building the target model's training data (never filtered out there).
# 0=no, 1=yes, 2=unknown, 3=suppressed (*)
...
def get_utility_target_model(df_train_real_raw, df_test_real_raw, df_protected_original, seed, drop_sens=False):
    ...
    df_train_real = df_train_real[df_train_real[PREDICTION_TARGET].astype(str) != "*"].dropna(subset=[PREDICTION_TARGET]).copy()
    df_test_real = df_test_real[df_test_real[PREDICTION_TARGET].astype(str) != "*"].dropna(subset=[PREDICTION_TARGET]).copy()
    ...
    if df_protected_original is None:
        ...
        for drop_col in ["index", "duration"]:
            if drop_col in df_protected.columns: df_protected = df_protected.drop(columns=[drop_col])
        ...
        df_train = df_train[df_train[PREDICTION_TARGET].astype(str) != "*"].dropna(subset=[PREDICTION_TARGET]).copy()
        if len(df_train) == 0:
...
def build_scenario_b_query_features(df_train_real_raw, df_protected_original, FEATURE_COLS, train_medians, drop_sens=False):
    ...
    if df_protected_original is None:
        ...
        for c in ["index", "duration", "Linkage_Index"]:
            if c in df_train.columns: df_train = df_train.drop(columns=[c])
    else:
        df_protected = df_protected_original.copy()
        df_protected.columns = df_protected.columns.str.strip()
        for c in ["index", "duration"]:
            if c in df_protected.columns: df_protected = df_protected.drop(columns=[c])
        if "Linkage_Index" in df_protected.columns:
            df_protected = df_protected.set_index("Linkage_Index")
...
def encode_adversary_features(df, preprocessor=None, fit=False, filter_sensitive=True, df_train_real_raw=None):
    ...
    sens = sens_raw.map(sens_map).fillna(SUPPRESSED_CATEGORY_CODE).astype(int).values
...
majority_class_rate = df_test_real_raw[df_test_real_raw[SENSITIVE_ATTR].astype(str) != '*'][SENSITIVE_ATTR].astype(str).str.strip().str.lower().map({'no': 0, 'yes': 1, 'unknown': 2}).fillna(3).value_counts(normalize=True).max()
...
for seed in seeds:
    ...
    for name, df_adv_original in datasets_to_test.items():
        ...
        result = get_utility_target_model(df_train_real_raw, df_test_real_raw, df_pass, seed, drop_sens=False)
        if result[0] is None:
        ...
        FEATURE_COLS_B = [c for c in df_train_real_raw.columns if c not in ["index", "duration", "Linkage_Index", PREDICTION_TARGET]]
...
pd.DataFrame(ablation_results).to_csv("../results/aia/bank_marketing_aia_housing_ablation.csv", index=False)
