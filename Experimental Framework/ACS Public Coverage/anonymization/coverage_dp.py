...
df_to_test = {
    "k=3": pd.read_csv("../datasets/ARX_acs_public_coverage_k3.csv"),
    "k=5": pd.read_csv("../datasets/ARX_acs_public_coverage_k5.csv"),
}
...
for dataset_name, df_original in df_to_test.items():
    ...
    for epsilon in epsilons:
        ...
        out_path = f"{OUTPUT_DIR}{dataset_name}_epsilon_{str(epsilon).replace('.', '_')}_public_coverage.csv"
    ...
    for epsilon in epsilons:
        print(f"  epsilon={epsilon} -> {dataset_name}_epsilon_{str(epsilon).replace('.', '_')}_public_coverage.csv")