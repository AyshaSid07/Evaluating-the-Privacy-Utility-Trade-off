...
runs = [
    ...
    ('../datasets/folktables_public_coverage_train.csv', 'DP', [0.1, 0.5, 1.0, 3.0, 5.0, 10.0]),
    
    # 2. Layered Approach (k=3 + DP) using cleaned ARX output
    ('../datasets/ARX_acs_public_coverage_k3_cleaned.csv', 'combined_k=3', [0.5, 1.0, 3.0]),
    
    # 3. Layered Approach (k=5 + DP) using cleaned ARX output
    ('../datasets/ARX_acs_public_coverage_k5_cleaned.csv', 'combined_k=5', [0.5, 1.0, 3.0])
]
...
for input_file, prefix, epsilons in runs:
    ...
    for epsilon in epsilons:
        for realization in range(1, REALIZATIONS + 1):
            ...
            if prefix == 'DP':
                out_path = f"{OUTPUT_DIR}DP_epsilon_{eps_str}_r{realization}_public_coverage.csv"
            else:
                out_path = f"{OUTPUT_DIR}{prefix}_epsilon_{eps_str}_r{realization}_public_coverage.csv"
