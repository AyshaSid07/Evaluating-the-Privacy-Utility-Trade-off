...
mcc_data = {
    "240": {"mobile_prefixes": ["4670", "4672", "4673", "4676", "4679"], "mncs": ["01", "02", "04", "05", "07", "08", "16", "24"], "tz_offsets": ["+0100", "+0200"], "network_weights": [0.10, 0.30, 0.60], "device_weights": [0.70, 0.20, 0.10], "tmsi_range": (10000000, 40000000), "lmsi_range": (10000000, 35000000), "tlli_range": (10000000, 30000000)},
    "206": {"mobile_prefixes": ["3245", "3246", "3247", "3248", "3249"], "mncs": ["01", "05", "10", "20"], "tz_offsets": ["+0100", "+0200"], "network_weights": [0.15, 0.35, 0.50], "device_weights": [0.60, 0.25, 0.15], "tmsi_range": (30000000, 60000000), "lmsi_range": (30000000, 55000000), "tlli_range": (25000000, 55000000)},
    "234": {"mobile_prefixes": ["4473", "4474", "4475", "4477", "4478"], "mncs": ["10", "15", "20", "30", "33", "38"], "tz_offsets": ["+0000", "+0100"], "network_weights": [0.50, 0.35, 0.15], "device_weights": [0.40, 0.35, 0.25], "tmsi_range": (50000000, 80000000), "lmsi_range": (50000000, 75000000), "tlli_range": (50000000, 78000000)},
    "310": {"mobile_prefixes": ["1202", "1305", "1415", "1512", "1617"], "mncs": ["120", "260", "410", "480"], "tz_offsets": ["-0500", "-0600", "-0700", "-0800"], "network_weights": [0.20, 0.55, 0.25], "device_weights": [0.50, 0.15, 0.35], "tmsi_range": (70000000, 99999999), "lmsi_range": (70000000, 95000000), "tlli_range": (65000000, 99999999)}
}

for mcc, country in mcc_data.items():
    country["ip_pool"] = [fake.ipv4() for _ in range(300)]
    country["ip_weights"] = [10, 5, 3, 1, 1] + [1] * 295  
    
    country["lac_pools"] = {mnc: [random.randint(1000, 9999) for _ in range(80)] for mnc in country["mncs"]}
    country["lac_weights"] = [20, 10, 5, 5, 5] + [1] * 75  
    
    country["rac_pool"] = [random.randint(10, 99) for _ in range(15)]
    country["rac_weights"] = [15, 10, 5] + [1] * 12
...
for i in range(rows):
    ...
    if i < outliers_count:
        ...
        dt = fake.date_time_between(start_date='-7d', end_date='now')
        
        lac = random.choices(country["lac_pools"][MNC], weights=country["lac_weights"], k=1)[0]
        rac = random.choices(country["rac_pool"], weights=country["rac_weights"], k=1)[0]
        ip = random.choices(country["ip_pool"], weights=country["ip_weights"], k=1)[0]
    ...
    data.append([
        imsi, msisdn, imei, ip, timestamp,
        MCC, MNC, PLMN, tmsi, lmsi, tlli,
        lai, rai, network, device, linkage_index
    ])
...
columns = [
    "IMSI", "MSISDN", "IMEI", "IP_Address", "Timestamp",
    "MCC", "MNC", "PLMN", "TMSI", "LMSI", "TLLI",
    "LAI", "RAI", "Network_Type", "Device_Type", "Linkage_Index"
]
