                ...
                for val_p in unique_protected_vals:
                    ...
                    if str_p.startswith('[') and str_p.endswith('[') or str_p.startswith('[') and str_p.endswith(']'):
                        ...
                        if lower <= val_a_float < upper:
                            ...
                            break
                    elif str_p.startswith('>') or str_p.startswith('<'):
                            val_a_float = float(val_a) # Hackerns exakta siffra
                            if str_p.startswith('>='):
                                limit = float(str_p[2:].strip())
                                if val_a_float >= limit: df_attacker_aligned.at[index, col] = str_p; break
                            elif str_p.startswith('>'):
                                limit = float(str_p[1:].strip())
                                if val_a_float > limit: df_attacker_aligned.at[index, col] = str_p; break
                            elif str_p.startswith('<='):
                                limit = float(str_p[2:].strip())
                                if val_a_float <= limit: df_attacker_aligned.at[index, col] = str_p; break
...
if __name__ == "__main__":
    ...
    datasets_to_test = {
        ...
        # "Anjana Bank Marketing k = 5": pd.read_csv('../datasets/bank_marketing_k=5.csv'),
        # "Anjana Bank Marketing k = 5, l = 2": pd.read_csv('../datasets/bank_marketing_k=5_l=2.csv'),
        # "Anjana Bank Marketing k = 5, l = 3": pd.read_csv('../datasets/bank_marketing_k=5_l=3.csv'),
        # "Anjana Bank Marketing k = 5, t = 0.10": pd.read_csv('../datasets/bank_marketing_k=5_t=0.1.csv'),
        # "Anjana Bank Marketing k = 5, t = 0.05": pd.read_csv('../datasets/bank_marketing_k=5_t=0.05.csv'),
        "Anjana Bank Marketing k = 2": pd.read_csv('../datasets/bank_marketing_k=10.csv'),
        "Anjana Bank Marketing k = 5": pd.read_csv('../datasets/bank_marketing_k=10_l=2.csv'),
        "Anjana Bank Marketing k = 10": pd.read_csv('../datasets/bank_marketing_k=10_l=3.csv'),
        "Anjana Bank Marketing k = 20": pd.read_csv('../datasets/bank_marketing_k=10_t=0.1.csv'),
        "Anjana Bank Marketing k = 50": pd.read_csv('../datasets/bank_marketing_k=10_t=0.05.csv'),
        # "Anjana Bank Marketing k = 5": pd.read_csv('../datasets/bank_marketing_k=5.csv'),
        # "Anjana Bank Marketing k = 5, l = 2": pd.read_csv('../datasets/bank_marketing_k=5_l=2.csv'),
        # "Anjana Bank Marketing k = 5, t = 0.10": pd.read_csv('../datasets/bank_marketing_k=5_t=0.1.csv'),
    }
