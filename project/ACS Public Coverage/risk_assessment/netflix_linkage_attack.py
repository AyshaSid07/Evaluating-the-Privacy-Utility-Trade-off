...
def plot_results(results_dict):
    ...
    plt.tight_layout()
    plt.savefig("../plots/netflix_attack_accuracy_acs_public_coverage.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    quasi_identifiers = ['AGEP', 'SCHL', 'MAR', 'SEX', 'CIT', 'RAC1P']


    datasets_to_test = {
        "Baseline (no anonymization)": df_original,
        "ARX ACS Public Coverage k-Anonymity k=10":          pd.read_csv('../datasets/ARX_public_coverage_k=10.csv'),
    ...
    for defense_name, df_protected in datasets_to_test.items():
        ...
        attacker_indices = df_protected_sample['Linkage_Index'].sample(n=1400, random_state=123)
