...
datasets_to_test = {
    "No anonymization (Baseline)": pd.read_csv('../datasets/folktables_public_coverage_RAW.csv'),
    "ARX Public Coverage k = 10": pd.read_csv('../datasets/ARX_public_coverage_k=10.csv'), 
    "ARX Public Coverage k = 20": pd.read_csv('../datasets/ARX_public_coverage_k=20.csv'),
    "ARX Public Coverage k = 50": pd.read_csv('../datasets/ARX_public_coverage_k=50.csv'),
...
def plot_utility_results(results_df):
    ...
    plt.title('Utility Evaluation on ACS Public Coverage Data (Logistic Regression)', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig("../plots/utility_public_coverage.png", dpi=300)
    plt.show()
