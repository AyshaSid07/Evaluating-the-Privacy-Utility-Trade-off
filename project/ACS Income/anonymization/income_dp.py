...
categorical_cols = ['AGEP', 'COW', 'SCHL', 'MAR', 'OCCP', 'POBP', 
                    'RELP', 'SEX', 'RAC1P', 'PINCP']
...
for epsilon in epsilons:
    ...
    out_path = f"{OUTPUT_DIR}income_dp_epsilon_{str(epsilon).replace('.', '_')}.csv"
...
for epsilon in epsilons:
    print(f"  epsilon={epsilon} -> income_dp_epsilon_{str(epsilon).replace('.', '_')}.csv")