...
for dataset_name, df_original in df_to_test.items():
    ...
    for epsilon in epsilons:
        ...
        out_path = f"{OUTPUT_DIR}{dataset_name}_epsilon_{str(epsilon).replace('.', '_')}_income.csv"
    ...
    for epsilon in epsilons:
        print(f"  epsilon={epsilon} -> {dataset_name}_epsilon_{str(epsilon).replace('.', '_')}_income.csv")