...
def plot_results(results_dict):
    ...
    # plt.title('Average Distance to Closest Record (Mean DCR) \nAcross different epsilon values of Differential Privacy on the ACS Employment Dataset', fontsize=13, pad=15)
    # plt.title('Average Distance to Closest Record (Mean DCR) \nAcross different k-values of k-Anonymity on the ACS Employment Dataset', fontsize=10)
    plt.title('Average Distance to Closest Record (Mean DCR) \nAcross different k-, l- and t-values of k-Anonymity, l-Diversity and t-Closeness on the ACS Employment Dataset', fontsize=10)
    plt.grid(axis='y', linestyle='--', alpha=0.7, zorder=0)
    # plt.ylim(0, 1)
    plt.tight_layout()
    # plt.savefig("../plots/dcr_mean_distance_acs_employment_DP.png", dpi=300)
    # plt.savefig("../plots/dcr_mean_distance_acs_employment_ARX_k_only.png", dpi=300)
    plt.savefig("../plots/dcr_mean_distance_acs_employment_ARX_k_l_t.png", dpi=300)
    plt.show()
...
csv_filepath = "../plots/dcr/dcr_results_acs_employment.csv" 
