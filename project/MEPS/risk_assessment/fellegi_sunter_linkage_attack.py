...
def perform_attack(df_protected, df_attacker_aligned, quasi_identifiers): 
    # Säkerställ att indexen har namn, även om vi kommer använda level=0 senare för säkerhets skull
    ...
    # FIX 1: Använd df_attacker_aligned här istället för df_attacker
    candidate_links = indexer.index(df_attacker_aligned, df_protected) 
    ...
    # FIX 2: Använd level=0 istället för namnet 'attacker_idx'
    # match_probs.index är en MultiIndex (attacker_idx, protected_idx)
    # level 0 är alltid den första DataFramen (attackern)
    attacker_levels = match_probs.index.get_level_values(0)
    
    for attacker_idx in df_attacker_aligned.index:
        if attacker_idx in attacker_levels:
            # Hämta alla sannolikheter för just denna attacker_idx
            probs_for_attacker = match_probs.loc[attacker_idx]
            
            # idxmax() ger oss den protected_idx som hade högst sannolikhet
...
if __name__ == "__main__":
    ...
    datasets_to_test = {
        ...
        "k-anonymity (k=2)":          pd.read_csv('../datasets/public_coverage_k=2.csv'),
        "k-anonymity (k=5)":          pd.read_csv('../datasets/public_coverage_k=5.csv'),
        "k-anonymity (k=10)":          pd.read_csv('../datasets/public_coverage_k=10.csv'),
        "k-anonymity (k=20)":          pd.read_csv('../datasets/public_coverage_k=20.csv'),
        "k-anonymity (k=50)":          pd.read_csv('../datasets/public_coverage_k=50.csv')
