...
mcc_data = {
    "240": {  # Sweden (CET)
        ...
        # --- NEW: country-specific distributions ---
        "network_weights": [0.10, 0.30, 0.60],   # heavily 5G
        "device_weights": [0.70, 0.20, 0.10],     # mostly smartphones
    ...
    "206": {  # Belgium (CET)
        ...
        "mncs": ["01", "05", "10", "20"],
        "tz_offsets": ["+0100", "+0200"],
        "network_weights": [0.15, 0.35, 0.50],   # skewed toward 5G
    ...
    "234": {  # UK (GMT/BST)
        ...
        "network_weights": [0.50, 0.35, 0.15],   # mostly 3G
    ...
    "310": {  # USA (multiple TZ)
        ...
        "network_weights": [0.20, 0.55, 0.25],   # mostly 4G
        "device_weights": [0.50, 0.15, 0.35],     # more IoT
...
for i in range(rows):
    ...
    # --- CORRELATED selections instead of random.choice ---
    ...
    # --- CORRELATED ranges instead of global random ---
...
df.to_csv("newtelecom_dataset.csv", index=False)
